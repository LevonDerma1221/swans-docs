...
## Risk committee
Members: head of risk (chair), CTO, CEO, one independent with clearing experience. Approves margin and fee parameters, model changes, offset-stage changes, liquidity-provider relief criteria and any intraday parameter action in stress. Reviews backtests monthly and validation annually.

## Model governance
Every model (pricing, hazard, margin, fee, marks filter) has an owner, a version, validation status, backtest history and a challenger. Changes follow change management; independent validation before production for margin and marks; documentation shared with PBs.
