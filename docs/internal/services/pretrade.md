...
2. `1 <= price <= 199`; `qty >= min_qty`; `qty <= max_order_qty`.
3. Static price band vs reference price (filtered fair mark, or last trade if newer): default 40 ticks.
4. Position limit: `|net_qty + signed_qty| <= position_limit`.
5. Balance check: `available >= max_loss`, where `max_loss = price * payout * qty` (buy) or `(1-price) * payout * qty` (sell). Reject with `INSUFFICIENT_BALANCE` (1055) if insufficient. Uses cached balance snapshot from `collateral.balances` stream.
...
- Fast upper bound first: `qty * payout * max(p, 1-p)`; precise margin call only if the bound fails
