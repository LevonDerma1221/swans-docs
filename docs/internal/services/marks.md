...
Produces the official filtered fair mark `X_fair` per contract, continuously intraday and at the 17:00 snapshot.
...
**Outputs:** `X_fair` stream to margin, positions (VM), price file, settlement (daily settle candidate), Product A. Every mark carries its component weights for audit.
