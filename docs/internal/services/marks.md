...
Produces the official filtered fair mark `X_fair` per contract, continuously intraday and at periodic snapshots.
...
**Outputs:** `X_fair` stream to risk engine (shadow mode), price file, settlement (reference mark candidate). Every mark carries its component weights, confidence, staleness and snapshot ID for audit.
