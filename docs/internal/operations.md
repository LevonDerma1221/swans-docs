...
|---|---|---|
| **Start of day** | 07:00 London | Verify journals replayed and epochs consistent; reference data snapshot published; PB adapter heartbeat; PB limits loaded; margin budgets pushed; market data snapshot; open at 08:00 |
| **End of day** | 17:00-18:30 | Fix marks and daily settle; margin run; PB position report ingest; reconciliation; files published; RTS 22/24 extracts; surveillance EOD feed; backup verification |
...
| **Trade bust** | Member request within 15 min or operator | Review vs erroneous-trade policy; decision by two staff; `ExecType=H` with reason; PBs notified; positions reverted; fee reversal; member notice |
| **Settlement determination** | Source publication | Officer 1 proposes with evidence hash; Officer 2 confirms; dispute window; committee if disputed; publish; PBs notified |
| **PB adapter down** | Notification failures or heartbeat loss | Halt new orders after 30 s; existing matched trades queued; escalate to PB; resume when link restored and queue drained; if beyond 15 min, member notice and FCA notification |
